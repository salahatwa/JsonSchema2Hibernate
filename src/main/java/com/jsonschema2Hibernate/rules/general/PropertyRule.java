...
import com.jsonschema2Hibernate.settings.rules.RulesNames;
 ...
 */
public class PropertyRule extends Rule<JDefinedClass, JDefinedClass> {
    ...
    public JDefinedClass apply(String nodeName, JsonNode node, JDefinedClass jclass, Schema schema) {
        ...
        if(node.has("MTM"))
        {
        	  ruleFactory.getAssociationMappingRule().apply(nodeName, node.get("MTM"), field,jclass, schema);
        }
    ...
    /**
     * Adding Role Property Annotation
     * @param nodeName
     * @param node
     * @param schema
     * @param generatedJavaConstruct
     */
    private void propertyAnnotations(String nodeName, JsonNode node, Schema schema, JDocCommentable generatedJavaConstruct) {
        ...
        if (node.has(JAVA_NAME)) {
            ruleFactory.getJavaNameRule().apply(nodeName, node.get(JAVA_NAME), generatedJavaConstruct, schema);
        }
        ...
        if(node.has(UNIQUE_COLUMN))
        {
        	 ruleFactory.getUniqueColumnRule().apply(nodeName, node.get(UNIQUE_COLUMN), generatedJavaConstruct, schema);
        }
        
        if(node.has(AUTOICREMENT_UNIQUE_COLUMN))
        {
        	 ruleFactory.getAutoIncrementUniqueColumnRule().apply(nodeName, node.get(AUTOICREMENT_UNIQUE_COLUMN), generatedJavaConstruct, schema);
        }
    }
