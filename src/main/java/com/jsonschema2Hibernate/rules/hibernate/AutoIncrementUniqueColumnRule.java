...
/**
 * Applies the "autoIncrementUniqueColumn" schema rule.
 */
public class AutoIncrementUniqueColumnRule extends Rule<JDocCommentable, JDocCommentable> {

	/**
	 * Applies This Rule Only when Field property 
	 * autoIncrementUniqueColumn value <code>true</code> 
	 * annotate Field With @Id
	 * annotate Field WIth @GeneratedValue
	 */
    ...
    public JDocCommentable apply(String nodeName, JsonNode node, JDocCommentable generatableType, Schema schema) {
    	...
    	if ( generatableType instanceof JFieldVar && node.asBoolean()) 
    	{
            ...
            ((JFieldVar) generatableType).annotate(GeneratedValue.class).param("strategy", GenerationType.AUTO);
    	}
